...
export default async function CreaturePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ salvo?: string }> }) {
  ...
  return <><Header /><main className={styles.detail}>
    ...
    <div className={styles.detailOverview}>
      ...
      <dl className={styles.facts}><div><dt>Categoria</dt><dd>{creature.category}</dd></div><div><dt>Nível de ameaça</dt><dd><ThreatStars level={creature.threat} /><span className={styles.threatCount}>{`${creature.threat} de 8`}</span></dd></div><div><dt>Responsável pela descoberta e informações</dt><dd>{responsible ? <Link href={`/funcionarios/${responsible.code}`}>{responsible.name}</Link> : "Não informado"}</dd></div></dl>
    </div>
