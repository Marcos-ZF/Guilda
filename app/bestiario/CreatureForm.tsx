  ...
  return <form action={action} className={styles.form}>
    ...
    <fieldset className={styles.wide}><legend>Habilidades</legend>
      {abilities.map((ability, index) => <div className={styles.abilityEditor} key={ability.key}>
        <label>Nome da habilidade {index + 1}<span className={styles.abilityNameInput}><DamageDot type={ability.damage_type} /><input name="ability_name" maxLength={100} required value={ability.name} onChange={event => setAbilities(items => items.map(item => item.key === ability.key ? { ...item, name: event.target.value } : item))} /></span></label>
        <input type="hidden" name="ability_damage_type" value={ability.damage_type ?? ""} />
        <fieldset className={styles.damagePicker}><legend>Categoria de dano</legend>{damageOptions.map(option => <label key={option.value}><input type="radio" name={`damage-${ability.key}`} value={option.value} checked={(ability.damage_type ?? "") === option.value} onChange={() => setAbilities(items => items.map(item => item.key === ability.key ? { ...item, damage_type: (option.value || null) as DamageType | null } : item))} /><DamageDot type={(option.value || null) as DamageType | null} /><span>{option.label}</span></label>)}</fieldset>
        ...
        <button className={styles.removeAbility} type="button" aria-label={`Remover habilidade ${index + 1}`} onClick={() => setAbilities(items => items.filter(item => item.key !== ability.key))}>× Remover</button>
      </div>)}
      {!abilities.length && <p className={styles.hint}>Nenhuma habilidade adicionada.</p>}
      <button type="button" disabled={abilities.length >= 20} onClick={() => setAbilities(items => [...items, { key: crypto.randomUUID(), name: "", description: "", damage_type: null }])}>+ Adicionar habilidade</button>
    </fieldset>
